...
class ir_mail_server(osv.osv):
    ...
    def send_email(self, cr, uid, message, mail_server_id=None, smtp_server=None, smtp_port=None,
        ...
        smtp_user = message['Return-Path']
        if not smtp_user:
            smtp_user = self._get_default_bounce_address(cr, uid, context=context)
        if not smtp_user:
            smtp_user = message['From']
        assert smtp_user, "The Return-Path or From header is required for any outbound email"

        # The email's "Envelope From" (Return-Path), and all recipient addresses must only contain ASCII characters.
        from_rfc2822 = extract_rfc2822_addresses(smtp_user)
        assert from_rfc2822, ("Malformed 'Return-Path' or 'From' address: %r - "
                              "It should contain one valid plain ASCII email") % smtp_user
        # use last extracted email, to support rarities like 'Support@MyComp <support@mycompany.com>'
        smtp_user = from_rfc2822[-1]
        email_to = message['To']
        email_cc = message['Cc']
